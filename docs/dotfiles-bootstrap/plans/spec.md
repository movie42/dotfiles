...
- **Apple Silicon / Intel 경로 차이**: Homebrew 위치가 `/opt/homebrew`와 `/usr/local`로 다르다. `$(brew --prefix)`로 받아서 쓰되, 셸 시작 때마다 부르지 않도록 install.sh에서 한 번 계산해 `zsh/path.zsh`가 참조할 값으로 남긴다.
